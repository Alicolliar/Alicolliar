...
func postMicro(post microPost, key string) error {
	var toPost bytes.Buffer
	post.Timestamp = time.Now()
	enc := json.NewEncoder(&toPost)
	enc.Encode(post)
	req, err := http.NewRequest(http.MethodPost, BASE_URL+"/blog/new/micro", &toPost)
	if err != nil {
		log.Panicln("HTTP NewReq err", err)
		return err
	}
...
func main() {
	...
	aNewMicroPost := microPost{}
	app := tview.NewApplication()
	if *microPostOn {
		var someText string
		form = tview.NewForm().
			AddTextView("Posting to ", BASE_URL, 50, 0, false, false).
			AddTextView("Messages", someText, 50, 0, false, true).
			AddTextArea("Post text", "", 0, 25, 0, func(text string) { aNewMicroPost.PostText = text }).
			AddInputField("Image Id (Optional)", "", 50, nil, func(text string) { aNewMicroPost.ImageId = text }).
			AddButton("Submit Post", func() {
				if err := postMicro(aNewMicroPost, *key); err != nil {
					someText += "An error occured\n"
				}
				someText += "Post sent\n"
			})
	} else {
